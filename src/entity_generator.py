...
class Number(Entity):
    ...
    def write_base_method_contents(self, f):
        f.write("Base { value: BigUint::from(")
        f.write(str(self.parameters["base"]))
        f.write("usize)}")
    def write_encode_method_contents(self, f):
        f.write("write(number, &Digit { value: self.value.clone(), base: Self::base() } );")
    def write_decode_method_contents(self, f):
...
class String(Entity):
    def write_entity_type(self, f):
        f.write("struct")
    def write_entity_contents(self, f):
        f.write("pub contents: String")
    def write_base_method_contents(self, f):
        f.write("Base { value: BigUint::from(256usize) }")
    def write_encode_method_contents(self, f):
        f.write("write(number, &Digit { value: BigUint::ZERO, base: Self::base() } );")
        f.write("for byte in self.contents.as_bytes().iter().rev() {")
        f.write("write(number, &Digit { value: BigUint::from(*byte), base: Self::base() });")
        f.write("}")
    def write_decode_method_contents(self, f):
        ...
        f.write("}")
        f.write("Self { contents: String::from_utf8(bytes).unwrap() }")
...
class Enum(Entity):
    ...
    def write_encode_method_contents(self, f):
        ...
        for index, variant in enumerate(self.parameters["variants"]):
            ...
            f.write("write(number, &Digit { value: BigUint::from(")
            f.write(str(index))
            f.write("usize), base: Self::base()")
            f.write("} );")
            f.write("}")
        f.write("}")
    def write_decode_method_contents(self, f):
...
class Sequence(Entity):
    ...
    def write_encode_method_contents(self, f):
        ...
        f.write(self.parameters["content_type"])
        f.write("::base());")
        f.write("write(number, &Digit { value: item + 1usize, base: Self::base() });")
        f.write("}")
    def write_decode_method_contents(self, f):
        ...
        f.write("write(number, &Digit { value: num - 1usize, base:")
        f.write(self.parameters["content_type"])
        f.write("::base()")
        f.write("});")
        f.write("items.push(")
