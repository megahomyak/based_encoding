import jinja2
import json
import os

TEMPLATE = jinja2.Template(
"""
// This file was generated automatically from {{ input_file_name }}. DO NOT EDIT THIS FILE MANUALLY!
use crate::operations::{read, write};
use crate::biguint::BigUInt;
use std::marker::PhantomData;

{% for entity in entities %}
    #[derive(Debug, PartialEq, Eq)]
    {%- if entity.type == "number" %}
        pub struct {{ entity.name }}<N> {
            pub value: N,
        }

        impl<N: BigUInt> {{ entity.name }}<N> {
            #[allow(dead_code)]
            pub fn base() -> N {
                I::from({{ entity.base }}usize)
            }
            pub fn encode(&self, number: &mut N) {
                write(number, &Self::base(), &self.value);
            }
            pub fn decode(number: &mut N) -> Self {
                Self { value: read(number, &Self::base()) }
            }
        }
    {% elif entity.type == "string" %}
        pub struct {{ entity.name }}<N> {
            pub contents: std::string::String,
            _n: PhantomData<N>,
        }

        impl<BigUInt: N> {{ entity.name }}<N> {
            #[allow(dead_code)]
            pub fn base() -> N {
                N::from(256usize)
            }
            pub fn encode(&self, number: &mut N) {
                write(number, &Self::base(), &N::from(0usize));
                for byte in self.contents.as_bytes().iter().rev() {
                    write(number, &Self::base(), &N::from(*byte as usize));
                }
            }
            pub fn decode(number: &mut N) -> Self {
                let mut bytes = Vec::new();
                loop {
                    let byte = read(number, &Self::base());
                    if byte == N::from(0usize) {
                        break;
                    }
                    bytes.push(byte.try_into_u8().unwrap());
                }
                Self { contents: std::string::String::from_utf8(bytes).unwrap() }
            }
        }
    {% elif entity.type == "struct" %}
        pub struct {{ entity.name }}<N> {
            {% for field in entity.fields -%}
                pub {{ field.name }}: {{ field.type }}<N>,
            {% endfor %}
        }

        impl<N: BigUInt> {{ entity.name }}<N> {
            #[allow(dead_code)]
            pub fn base() -> N {
                {{ entity.fields[0].type }}::base::<N>()
            }
            pub fn encode(&self, number: &mut N) {
                {% for field in entity.fields|reverse -%}
                    self.{{ field.name }}.encode(number);
                {% endfor %}
            }
            pub fn decode(number: &mut N) -> Self {
                {% for field in entity.fields -%}
                    let {{ field.name }} = {{ field.type }}::decode(number);
                {% endfor -%}
                Self {
                    {% for field in entity.fields -%}
                        {{ field.name }},
                    {% endfor %}
                }
            }
        }
    {% elif entity.type == "enum" %}
        pub struct {{ entity.name }}

        pub enum {{ entity.name }}Kind{% if entity|any_associated_types %}<N>{% endif %} {
            {% for variant in entity.variants -%}
                {{ variant.name }}(
                    {% if variant.associated_type != None %}
                        {{ variant.associated_type }}<N>
                    {% endif %}
                ),
            {% endfor %}
        }

        impl<N: BigUInt> {{ entity.name }}<N> {
            #[allow(dead_code)]
            pub fn base<N: BigUInt>() -> N {
                N::from({{ entity.variants|length }}usize)
            }
            pub fn encode<N: BigUInt>(&self, number: &mut N) {
                match self {
                    {% for variant in entity.variants -%}
                        Self::{{ variant.name }}(
                            {% if variant.associated_type != None %}
                                associated_value
                            {% endif %}
                        ) => {
                            {% if variant.associated_type != None %}
                                associated_value.encode(number);
                            {% endif -%}
                            write(number, &Self::base(), &N::from({{ loop.index0 }}usize));
                        }
                    {% endfor %}
                }
            }
            pub fn decode<N: BigUInt>(number: &mut N) -> Self {
                let num = read(number, &Self::base());
                {% for variant in entity.variants -%}
                    if num == N::from({{ loop.index0 }}usize) {
                        {% if variant.associated_type != None %}
                            let associated_value = {{ variant.associated_type }}::decode(number);
                        {% endif -%}
                        return Self::{{ variant.name }}(
                            {% if variant.associated_type != None %}
                                associated_value
                            {% endif %}
                        );
                    }
                {% endfor -%}
                unreachable!()
            }
        }
    {% elif entity.type == "sequence" %}
        pub struct {{ entity.name }} {
            pub items: Vec<{{ entity.content_type }}>,
        }

        impl {{ entity.name }} {
            #[allow(dead_code)]
            pub fn base<N: BigUInt>() -> N {
                {{ entity.content_type }}::base::<N>().add(&N::from(1usize))
            }
            pub fn encode<N: BigUInt>(&self, number: &mut N) {
                write(number, &Self::base(), &N::from(0usize));
                for item in self.items.iter().rev() {
                    item.encode(number);
                    let item = read(number, &{{ entity.content_type }}::base());
                    write(number, &Self::base(), &(item.add(&N::from(1usize))));
                }
            }
            pub fn decode<N: BigUInt>(number: &mut N) -> Self {
                let mut items = Vec::new();
                loop {
                    let num = read(number, &Self::base());
                    let Some(decremented) = num.subtract(&N::from(1usize)) else {
                        break;
                    };
                    write(number, &{{ entity.content_type }}::base(), &decremented);
                    items.push({{ entity.content_type }}::decode(number));
                }
                Self { items }
            }
        }
    {% endif %}
{% endfor %}
""".strip())
TEMPLATE.environment.filters["any_associated_types"] = lambda entity: any(
    "associated_type" in variant
    for variant in entity["variants"]
)

def main():
    input_file_name = "entities.json"
    with open(input_file_name, "r", encoding="utf-8") as f:
        entities = json.load(f)

    with open("entities.rs", "w", encoding="utf-8") as f:
        f.write(TEMPLATE.render(
            entities=entities,
            input_file_name=input_file_name,
        ))

    os.system("rustfmt entities.rs")

main()
